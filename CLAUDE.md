...
**Branded email + voice mode (also built):** Gmail drafts (`gmail_create_draft`) now render as a branded dark-themed HTML email (`services/connectors/gmail/template.py`) with the NOBS AI logo/header, alongside a plain-text fallback — one template for now, more can be added once Nobert has seen it. The assistant chat also has a voice mode toggle (`apps/web/src/hooks/useSpeechSynthesis.ts`, wired into `Assistant.tsx`): replies are read aloud via the browser's free `window.speechSynthesis`, and talking into the mic while voice mode is on auto-sends the message (no manual Send tap) and interrupts any reply currently playing — approximating a phone-call feel. Both free (browser-native TTS, no API); no new cost.

**Still not built:** video file analysis (Claude's API has no native video input — would need frame-extraction via ffmpeg as a workaround); a Claude-Code-like autonomous coding agent with its own persistent file system across turns (the current coding help is conversational + code_execution's sandbox — each call gets a container, not a standing dev environment); an in-app "clone a repo and preview it locally before pushing live" feature (would need a container-orchestration/sandboxing service comparable to CodeSandbox/StackBlitz — real infra, cost, and security implications; not attempted as a fake/partial version). PDF generation as an output is built (see the Assistant chat entry above).
