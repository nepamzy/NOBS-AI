...
**Not yet built (explicitly deferred, not silently skipped):** video file analysis (Claude's API has no native video input — would need frame-extraction via ffmpeg as a workaround); a Claude-Code-like autonomous coding agent with its own file system/git access (the current coding help is conversational + code_execution's sandbox, not a repo-aware agent — see chat history on 2026-09-17 for the honest cost/scope tradeoff on this); persistent cross-session memory; external connectors (Gmail draft, GitHub push, Vercel deploy) — each needs Nobert to set up the OAuth app/token himself first (CLAUDE.md Part 1: connecting an external account is a stop-and-ask action), plus a scoped decision on what the AI is allowed to do autonomously vs. only propose; and a security-hardening pass (rate limiting, etc.) beyond what Render/Vercel already provide by default.
