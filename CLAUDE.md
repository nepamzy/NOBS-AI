...
**Admin-only general assistant capability (also built, per Nobert's explicit instruction):** when `user.role == ADMIN`, the same `/chat/messages` endpoint adds the real `web_search` server tool ($0.01/search + normal input tokens for results) and a broadened system prompt covering open-ended research, coding help/teaching, and defensive cybersecurity guidance — still not exposed to guest users, who keep the narrow video-only assistant. Also accepts one image or PDF attachment per message (native Claude vision/document input, billed as normal input tokens — no extra cost tier) for the admin to ask questions about. **Not yet built:** video file analysis (Claude's API has no native video input — would need frame-extraction via ffmpeg as a workaround) and PDF *generation* as an output (would need Claude's code-execution + Files API, a materially bigger integration than input analysis) — both explicitly deferred, not silently skipped.
